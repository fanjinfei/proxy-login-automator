...
function main() {
  ...
  cfg.buf_proxy_basic_auth = new Buffer.from('Proxy-Authorization: Basic ' + new Buffer.from(cfg.usr + ':' + cfg.pwd).toString('base64'));
...
var CR = 0xd, LF = 0xa, BUF_CR = new Buffer.from([0xd]), BUF_CR_LF_CR_LF = new Buffer.from([0xd, 0xa, 0xd, 0xa]), BUF_LF_LF = new Buffer.from([0xa, 0xa]);
